...
class _CarpentryPageState extends State<CarpentryPage> {
  late Stream<QuerySnapshot> _carpentersStream;
  bool isLoading = true;
  Map<String, dynamic>? currentUserData;
  List<Map<String, dynamic>> availableInRangeCarpenters = [];
  List<Map<String, dynamic>> unavailableInRangeCarpenters = [];
  List<Map<String, dynamic>> outOfRangeCarpenters = [];

  @override
  void initState() {
    super.initState();
    _loadCurrentUserData();
    // Get all carpenters initially
    _carpentersStream = FirebaseFirestore.instance
        .collection('users')
        .where('partnerType', isEqualTo: 'Carpenter')
        .snapshots();
  }
  ...
  void _filterCarpenters(List<QueryDocumentSnapshot> carpenterDocs) {
    availableInRangeCarpenters.clear();
    unavailableInRangeCarpenters.clear();
    outOfRangeCarpenters.clear();

    if (currentUserData == null) {
      // If no user data, put all in out of range
      for (var doc in carpenterDocs) {
        var data = doc.data() as Map<String, dynamic>;
        outOfRangeCarpenters.add(data);
      }
      return;
    }
    ...
    if (customerLat == null || customerLng == null) {
      ...
      for (var doc in carpenterDocs) {
        ...
        if (isAvailable) {
          availableInRangeCarpenters.add(data);
        } else {
          unavailableInRangeCarpenters.add(data);
        }
      }
      return;
    }

    // Filter carpenters based on location and availability
    for (var doc in carpenterDocs) {
      var data = doc.data() as Map<String, dynamic>;
      bool isAvailable = data['isAvailable'] ?? false;
      double carpenterLat = 0.0;
      double carpenterLng = 0.0;
      double serviceRadiusKm = 0.5; // Default smaller value, was 5.0 before

      // Extract location data
      if (data.containsKey('locationDetails')) {
        carpenterLat = data['locationDetails']['latitude']?.toDouble() ?? 0.0;
        carpenterLng = data['locationDetails']['longitude']?.toDouble() ?? 0.0;
      }
      ...
      // Categorize carpenters into three groups
      if (isWithinRadius) {
        if (isAvailable) {
          availableInRangeCarpenters.add(data);
        } else {
    ...
    availableInRangeCarpenters.sort((a, b) {
      double distA = double.tryParse(a['distance'] ?? '0.0') ?? 0.0;
      double distB = double.tryParse(b['distance'] ?? '0.0') ?? 0.0;
      return distA.compareTo(distB);
    });

    unavailableInRangeCarpenters.sort((a, b) {
      double distA = double.tryParse(a['distance'] ?? '0.0') ?? 0.0;
      double distB = double.tryParse(b['distance'] ?? '0.0') ?? 0.0;
      return distA.compareTo(distB);
    });

    outOfRangeCarpenters.sort((a, b) {
      double distA = double.tryParse(a['distance'] ?? '0.0') ?? 0.0;
      double distB = double.tryParse(b['distance'] ?? '0.0') ?? 0.0;
      return distA.compareTo(distB);
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: Colors.brown[400], // Wood-inspired color
        centerTitle: true,
        elevation: 4,
        leading: IconButton(
      ...
      body: isLoading
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data == null || snapshot.data!.docs.isEmpty) {
            return Center(child: Text('No carpenters found'));
          }

          // Filter carpenters based on location
          _filterCarpenters(snapshot.data!.docs);
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Available In Range Carpenters Section
                Text(
                  "Available (In Range) - ${availableInRangeCarpenters.length}",
                  style: TextStyle(
                    fontSize: 18,
                ...
                SizedBox(height: 12),
                availableInRangeCarpenters.isEmpty
                    ? Padding(
                  padding: EdgeInsets.symmetric(vertical: 20),
                  child: Center(
                    child: Text("No available carpenters in your area"),
                  ),
                )
                    : ListView.builder(
                  shrinkWrap: true,
                  physics: NeverScrollableScrollPhysics(),
                  itemCount: availableInRangeCarpenters.length,
                  itemBuilder: (context, index) {
                    return _buildCarpenterListItem(
                      availableInRangeCarpenters[index],
                      status: "Available",
                    );
                  },
                ),
                ...
                // Unavailable In Range Carpenters Section
                Text(
                  "Unavailable (In Range) - ${unavailableInRangeCarpenters.length}",
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                ...
                SizedBox(height: 12),
                unavailableInRangeCarpenters.isEmpty
                    ? Padding(
                  padding: EdgeInsets.symmetric(vertical: 20),
                  child: Center(
                    child: Text("No unavailable carpenters in your area"),
                  ),
                )
                    : ListView.builder(
                  shrinkWrap: true,
                  physics: NeverScrollableScrollPhysics(),
                  itemCount: unavailableInRangeCarpenters.length,
                  itemBuilder: (context, index) {
                    return _buildCarpenterListItem(
                      unavailableInRangeCarpenters[index],
                      status: "Unavailable",
                    );
                  },
                ),
                ...
                // Out of Range Carpenters Section
                Text(
                  "Out of Range - ${outOfRangeCarpenters.length}",
                  style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                ...
                SizedBox(height: 12),
                outOfRangeCarpenters.isEmpty
                    ? Padding(
                  padding: EdgeInsets.symmetric(vertical: 20),
                  child: Center(
                    child: Text("No carpenters out of range"),
                  ),
                )
                    : ListView.builder(
                  shrinkWrap: true,
                  physics: NeverScrollableScrollPhysics(),
                  itemCount: outOfRangeCarpenters.length,
                  itemBuilder: (context, index) {
                    return _buildCarpenterListItem(
                      outOfRangeCarpenters[index],
                      status: "Out of Range",
                    );
                  },
                ),
  ...
  Widget _buildCarpenterListItem(Map<String, dynamic> carpenter,
      {required String status}) {
    // Extract data with defaults
    String name = carpenter["name"] ?? "Unknown";
    String experience = carpenter["experience"] ?? "Not specified";
    String imageUrl = carpenter["profileImage"] ??
        "assets/images/profile image.png";
    String distance = carpenter["distance"] ?? "Unknown";
    bool isVerified = carpenter["verificationStatus"] == "verified";
      ...
      child: InkWell(
        onTap: () =>
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    CarpenterDetailsPage(carpenter: carpenter),
              ),
            ),
        child: Padding(
          padding: EdgeInsets.all(12),
            ...
            children: [
              ...
              if (isVerified)
                Padding(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.verified_user, size: 14, color: Colors.brown[400]),
                      SizedBox(width: 4),
                      Text(
...
class CarpenterDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    String name = carpenter["name"] ?? "Unknown";
    String partnerType = carpenter["partnerType"] ?? "Carpenter";
    bool available = carpenter["isAvailable"] ?? false;
    bool isVerified = carpenter["verificationStatus"] == "verified";
    String verificationDate = carpenter["verificationDate"] ?? "";

    // Store the actual rating value
    double? ratingValue = carpenter["rating"]?.toDouble();
    ...
    String imageUrl = carpenter["profileImage"] ?? "assets/images/profile image.png";
    String distance = carpenter["distance"] ?? "Unknown";
    String experience = carpenter["experience"] ?? "Not specified";
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                if (isVerified) Icon(Icons.verified, size: 24, color: Colors.brown[400]),
              ],
            ),
            ...
            if (distance != "Unknown")
              Chip(
                ...
                avatar: Icon(Icons.location_on, size: 20, color: Colors.brown[700]),
                label: Text(
                  "$distance km away",
                    ...
                    if (isVerified)
                      Column(
                        children: [
                          Row(
                            children: [
                              Icon(Icons.verified_user, color: Colors.brown[400]),
                              SizedBox(width: 12),
                              Text(
                                "Verified Professional",
                                style: TextStyle(
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: available ? Colors.brown[400] : Colors.brown[300],
                padding: EdgeInsets.symmetric(horizontal: 50, vertical: 15),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
              ),
              child: Text(
                available ? "Book Carpenter" : "Book for Later",
  ...
  Widget _buildInfoRow(IconData icon, String label, String value) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Icon(icon, size: 22, color: Colors.brown[700]),
        SizedBox(width: 12),
        Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
